<template>
    <div class="wpsr-all-reviews" :class="reviewsWrapperClasses">
      <div :class="reponsiveClasses" v-for="(review, index) in latestReviews" :key="index">
          <div class="wpsr-review-template wpsr-review-template-three"
               :data-index="index"
          ...
          >
              <div class="wpsr-review-platform" v-if="(review.platform_name !== 'ai' && review.platform_name !== appVars.tp_slug && template_meta.isPlatformIcon === 'true') || (template_meta.isPlatformIcon === 'true' && template_meta.display_tp_brand === 'true' && review.platform_name === appVars.tp_slug)">
                  <span>{{ getPlatform(review.platform_name, template_meta) }}</span>
              </div>
              <ReviewTitle :template_meta="template_meta" :review="review"></ReviewTitle>
              <transition name="fade" v-if="review.platform_name !== 'ai'">
                  <ReviewRating
    ...
    <script type="text/babel">
        ...
        export default {
            ...
            methods: {
                ...
                getPlatform(platform, template_meta) {
                  let platforms = ['custom', 'fluent_forms', 'woocommerce'];
                  if(!platforms.includes(platform) || !template_meta.platform_label.length) {
                    return this.$getFirstWord(platform);
                  }
    
                  return template_meta.platform_label;
                },
                expandContent(val) {
