<template>
    <div v-if="feed_config" class="wpsr-connection-platform wpsr-feed-connection-platform">
        ...
        <div class="wpsr-platforms-group">
            <div class="wpsr-platforms-list-group wpsr-connection-wrapper">
                <div class="wpsr-platform wpsr-platform--connection" :style="connectionCardStyle">
                    <div class="wpsr-platform-connection-row" @click="tiktokSettingModal = true">
                        <img :src="assets_url+'/images/icon/icon-tiktok.png'" class="image wpsr-platform-connection-icon">
                        ...
                        <span class="wpsr-platform-connection-gear" @click.stop="tiktokSettingModal = true">
        ...
        <tiktok-config
            v-if="tiktokSettingModal"
            :access_token="verificationData"
            :tiktokModal="tiktokSettingModal"
            @tiktok_settings="tiktokSettingModal = $event"
        >
        </tiktok-config>
    </div>
</template>
...
import TiktokConfig from '../../../views/platforms/feeds/Tiktok';
...
export default {
    ...
    data() {
        return {
            tiktokSettingModal: false,
            localFeedConfig: null,
            isUpdating: false,
            verificationData: '' // Add this to store access token
        }
    },

    computed: {
        isPlatformConnected() {
            // Check if TikTok is connected based on connected_ids or other connection indicators
    ...
    mounted() {
        ...
        // Add these lines to extract access_token from URL params
        let platform = urlParams.get('platform');
        let oauth_token = urlParams.get('access_token');
        
        if (oauth_token && platform === 'tiktok') {
        ...
        if (!configuration) {
            ...
            if (hashQuery) {
                ...
                // Also check hash params for access_token
                if (!oauth_token) {
