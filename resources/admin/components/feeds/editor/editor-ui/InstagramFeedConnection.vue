<template>
    <div v-if="feed_config" class="wpsr-connection-platform wpsr-feed-connection-platform">
        ...
        <div class="wpsr-platforms-group">
            <div class="wpsr-platforms-list-group wpsr-connection-wrapper">
                <div class="wpsr-platform wpsr-platform--connection" :style="connectionCardStyle">
                    <div class="wpsr-platform-connection-row" @click="instagramSettingModal = true">
                        <img :src="assets_url+'/images/icon/icon-instagram.png'" class="image wpsr-platform-connection-icon">
                        ...
                        <span class="wpsr-platform-connection-gear" @click.stop="instagramSettingModal = true">
        ...
        <instagram-config
            ref="instagramConfig"
            v-if="instagramSettingModal"
            :instagramModal="instagramSettingModal"
            :access_token="verificationData"
            :credentials_type="credentialsType"
            :expires_in="expiresIn"
            @instagram_settings="instagramSettingModal = $event"
        >
        </instagram-config>
    </div>
</template>
...
import InstagramConfig from '../../../views/platforms/feeds/Instagram';
...
export default {
    ...
    computed: {
        isPlatformConnected() {
            // Check if Instagram is connected based on connected_ids or other connection indicators
    ...
    mounted() {
        ...
        // Set Instagram parameters if they exist
        if (oauth_token && platform === 'instagram') {
            this.verificationData = oauth_token;
            this.credentialsType = credentials_type;
            this.expiresIn = expires_in;
        } else if (auth_code && platform === 'instagram') {
            this.verificationData = auth_code;
        }
        ...
        if(configuration){
            ...
            // Also open businessAccountsModal if it's a business account
            if (this.credentialsType === 'business') {
                // Wait for the Instagram component to be rendered and data loaded
                this.$nextTick(() => {
                    setTimeout(() => {
                        const instagramComponent = this.$refs.instagramConfig;
