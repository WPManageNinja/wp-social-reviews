<template>
    <div v-if="feed_config" class="wpsr-connection-platform wpsr-feed-connection-platform">
        ...
        <div class="wpsr-platforms-group">
            <div class="wpsr-platforms-list-group wpsr-connection-wrapper">
                <div class="wpsr-platform wpsr-platform--connection" :style="connectionCardStyle">
                    <div class="wpsr-platform-connection-row" @click="facebookSettingModal = true">
                        <img :src="assets_url+'/images/icon/icon-facebook-small.png'" class="image wpsr-platform-connection-icon">
                        ...
                        <span class="wpsr-platform-connection-gear" @click.stop="facebookSettingModal = true">
        ...
        <facebook-config
            v-if="facebookSettingModal"
            :facebookModal="facebookSettingModal"
            :access_token="verificationData"
            @facebook_settings="facebookSettingModal = $event"
        >
        </facebook-config>
    </div>
</template>
...
import FacebookConfig from '../../../views/platforms/feeds/Facebook';
...
export default {
    ...
    computed: {
        isPlatformConnected() {
            // Check if Facebook is connected based on connected_ids or other connection indicators
    ...
    mounted() {
        ...
        // Set Facebook parameters if they exist
        if (oauth_token && platform === 'facebook_feed') {
            this.verificationData = oauth_token;
        } else if (auth_code && platform === 'facebook_feed') {
        ...
        if(configuration){
            this.facebookSettingModal = true;
        }
    },
