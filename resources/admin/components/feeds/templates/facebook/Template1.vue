<template>
  <div>
    ...
    <div class="wpsr-fb-feed-wrapper wpsr-facebook-masonry-activate wpsr-fb-feed-template1" :class="[
      ...
      <div class="wpsr-container">
        ...
        >
          <p style="font-size: 18px" v-if="feedConfig.dynamic.items.length === 0 && !feedConfig.dynamic.error_message && feedConfig.feed_settings.source_settings.feed_type !== 'event_feed' && feedConfig.dynamic.header">{{ $t("Posts are not available!") }}</p>
          <div :class="[feedConfig.feed_settings.layout_type === 'carousel' && has_pro ? 'swiper-container' : '']">
            <div class="wpsr-fb-all-feed" :class="[
              ...
              <div class="wpsr-mb-30" :class="responsiveClasses" v-for="(feed, key) in feedItems" :key="key">
                <div class="wpsr-fb-feed-item wpsr-fb-post" :data-post_id="getPostId(feed, feedConfig.feed_type)" :data-user_name="feed.page_id" :data-image_size="feedConfig.feed_settings.post_settings.resolution">
                  <div v-if="feedConfig.feed_settings.source_settings.feed_type === 'timeline_feed'"
                    class="wpsr-fb-feed-inner">
                    <AuthorLinks :feed="feed" :account="feed.from"
                    ...
                    <div class="wpsr-fb-feed-content-wrapper wpsr-fb-feed-playmode"
                      @click="postDisplayMode($event, feed, feedConfig.feed_settings, key, image_settings)">
                      <Photo v-if="feed.status_type !== 'shared_story'" :feed="feed"
                        :feed_type="feedConfig.feed_settings.source_settings.feed_type"
                        :feed_settings="feedConfig.feed_settings.post_settings" :image_settings="image_settings" />
                  ...
                  <div v-else-if="has_pro && feedConfig.feed_settings.source_settings.feed_type === 'photo_feed'"
                    @click="postDisplayMode($event, feed, feedConfig.feed_settings, key, image_settings)">
                    <Photo :feed="feed" :feed_type="feedConfig.feed_settings.source_settings.feed_type"
                      :feed_settings="feedConfig.feed_settings.post_settings" :image_settings="image_settings" />
                  </div>
                  <div v-else-if="has_pro && feedConfig.feed_settings.source_settings.feed_type === 'video_feed' || feedConfig.feed_settings.source_settings.feed_type === 'video_playlist_feed'"
                    @click="postDisplayMode($event, feed, feedConfig.feed_settings, key, image_settings)">
                    <Videos :feed="feed" :feed_type="feedConfig.feed_settings.source_settings.feed_type"
                      :feed_settings="feedConfig.feed_settings.post_settings" :image_settings="image_settings" />
                  </div>
                  ...
                  <div v-else-if="has_pro && feedConfig.feed_settings.source_settings.feed_type === 'album_feed'"
                    @click="handleAlbumClick(feed)">
                    <Albums :feed="feed" :feed_type="feedConfig.feed_settings.source_settings.feed_type"
                      :feed_settings="feedConfig.feed_settings.post_settings" :image_settings="image_settings" v-if="!hasAlbumFeed" />
                  </div>
          ...
          <swiper-navs v-if="feedConfig.feed_settings.layout_type === 'carousel' && has_pro"
            :options="feedConfig.feed_settings.carousel_settings"></swiper-navs>
        </div>
        ...
        <Album v-if="has_pro && hasAlbumFeed" :feed="selectedAlbumFeed" :feedConfig="feedConfig" :image_settings="image_settings"
          @handleAlbumBreadCrumbs="handleAlbumBreadCrumbs" @breadCrumbs="hasAlbumFeed = false" @postDisplayMode="postDisplayMode" />

        <Footer :account="feedConfig.dynamic.header" :settings="feedConfig.feed_settings" />
      </div>

      <Popup
          v-if="selectedFeed && feedConfig.feed_settings.post_settings.display_mode === 'popup'"
          :feed="selectedFeed"
          :account="selectedFeed.from" :feedLength="feedConfig.dynamic.items ? feedConfig.dynamic.items.length : 0"
...
export default {
  inheritAttrs: false,
  mixins: [fbFeedMixin],
  props: ['image_settings', 'feedConfig', 'translations'],
  data() {
    return {
    }
  },
