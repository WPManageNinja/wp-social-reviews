...
class Config
{
    ...
    public function formatOldFeed($feed)
    {
        ...
        if(Arr::get($feed, 'text')) {
            $currentFeed['text'] = GlobalHelper::sanitizeForStorage(Arr::get($feed, 'text', ''));
        }

        if(Arr::get($feed, 'full_text')) {
            $currentFeed['text'] = GlobalHelper::sanitizeForStorage(Arr::get($feed, 'full_text', ''));
        }
...
//        }
    ...
    public function formatNewFeed($tweet, $mediaArr, $referencedTweetsArr, $userArr)
    {
        ...
        $currentTweet['text'] = GlobalHelper::sanitizeForStorage(Arr::get($tweet, 'text', ''));
        ...
        foreach ($referencedTweets as $referencedTweet) {
            ...
            if (!empty(Arr::get($referencedTweetsArr, $id))) {
                ...
                if($type == 'retweeted') {
                    $currentTweet['text'] = GlobalHelper::sanitizeForStorage(Arr::get($referencedTweetsArr, $id . '.text'));
