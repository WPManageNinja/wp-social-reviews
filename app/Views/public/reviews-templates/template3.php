...
if (!empty($reviews)) {
    foreach ($reviews as $wpsr_index => $wpsr_review) {
        ...
        ?>
            <div class="wpsr-review-template wpsr-review-template-three <?php echo ($wpsr_review->platform_name) ? 'wpsr-review-template-' . esc_attr($wpsr_review->platform_name) : ''; ?>"
            ...
            >
                <?php if ($wpsr_review->platform_name !== 'ai' && ($template_meta['isPlatformIcon'] === 'true' && !Helper::is_tp($wpsr_review->platform_name)) || ($template_meta['isPlatformIcon'] === 'true' && $template_meta['display_tp_brand'] === 'true' && Helper::is_tp($wpsr_review->platform_name ))) { ?>
                    <div class="wpsr-review-platform">
                        <span><?php echo esc_html((empty($template_meta['platform_label']) || !in_array($wpsr_review->platform_name, ['custom', 'fluent_forms', 'woocommerce'])) ? strtok($wpsr_review->platform_name, '_') : $template_meta['platform_label']); ?></span>
                    </div>
                <?php } ?>
                <?php
                /**
